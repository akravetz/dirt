...
_SYSTEM_PROMPT_APPEND = (
    ...
    "`wiki/overview.md` and `wi../README.md`. STEP 3: Run `uv run scripts/"
...
class ClaudeSynthesisRunner:
    ...
    def _build_prompt(
        ...
        return (
            ...
            f"{photo_lines}\n\n"
            "Use the Read tool to view each available image. Main-tent files "
            "are the preset plant/canopy views; files named like "
            "`breeding-overview.jpg` or `<tent>-overview.jpg` are overview "
            "photos for other tent sections. If an expected view is missing, "
            "explicitly note incomplete photo coverage and do not invent "
            ...
            f"```json\n{sensor_json}\n```\n\n"
            "Sensor JSON may include both legacy `tent` data and scoped "
            "`tents` data. Prefer `tents` when present. Include a concise "
            "breeding-tent section when `tents.breeding` or a breeding photo "
            "is available. Soil-moisture absolute calibration is not trusted; "
            "treat calibrated percentages as rough context only, emphasize "
            "relative movement/deltas, and only flag pinned, stale, missing, "
            "or large directional changes.\n\n"
            "WHAT TO PRODUCE\n"
            f"- `wiki/daily/{target_date.isoformat()}.md` with: per-plant "
            "photo observations, scoped tent updates including breeding when "
            "available, a sensor table covering the three windows, "
            "stage-appropriate recommendations, any flagged issues.\n"
            "- One-line Timeline entry on each current plant page listed by "
            "`wiki/AGENTS.md` and `wiki/overview.md`, using plant tag keys "
            "from the sensor JSON when identifying plants, plus a 1-2 "
            "sentence rewrite of Current State.\n"
            ...
            "Last Reading, Active Action Items) and `wi../README.md` (add "
            "today's daily link).\n"
...
class CodexSynthesisRunner:
    ...
    def _build_prompt(
        ...
        return (
            ...
            f"daily photos ({len(photo_paths)} available); main-tent files are "
            "preset plant/canopy views and files named like "
            "`breeding-overview.jpg` or `<tent>-overview.jpg` are other tent "
            "overview photos. The file paths are listed for identity/order:\n"
            f"{photo_lines}\n\n"
            "If an expected view is missing, explicitly note incomplete photo "
            "coverage and do not invent observations for missing views.\n\n"
            f"Sensor windows (overnight = 00-06 MDT, morning = 07-14 MDT, "
            f"now = at time of run; values where available, null otherwise):\n"
            f"```json\n{sensor_json}\n```\n\n"
            "Sensor JSON may include both legacy `tent` data and scoped "
