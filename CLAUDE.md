...
| `humidifier` | State transitions of the Kasa EP10 plug controlling the Raydrop humidifier. One `state_change` event per on/off change with `reason` (`vpd_above_upper_band` / `vpd_below_upper_band` / `failsafe_stale_sensor` / `outside_lights_window`), `vpd`, `vpd_age_s`, `stage`, `upper_band_kpa`, `lower_band_kpa`, `lights_on`, `minutes_until_off`, `minutes_until_on`, `allowed`. Also emits `error` events on loop exceptions and `suspected_stuck` events (once per continuous-ON streak) when the plug has been held ON for `humidifier_stuck_alert_after_s` without VPD dropping by `humidifier_stuck_min_vpd_drop_kpa` — the signature of a Raydrop low-water-sensor latch, also posted to Telegram. Loop targets the stage's VPD upper edge and forces off outside the allowed lights window (from `lights_on − margin` through `lights_off − margin`) — see `wiki/hardware/humidifier-control.md`. | 30 days | `apps/hwd/src/dirt_hwd/services/humidifier.py:HumidifierLoopService.run` |
| `humidifier_shadow` | Per-tick output of the shadow-mode PI controller (Phase 4 prep — does not actuate). One `tick` event per ~30 s loop iteration with `u_pct` (commanded intensity 0–100), `plug_on_shadow` vs `plug_on_actual`, `setpoint_kpa`, `error_kpa`, `p_term`, `i_term`, `integrator`, `reason` (`pi_active` / `failsafe_stale_sensor` / `outside_lights_window` / `rh_ceiling`), plus inputs (vpd, vpd_age_s, rh, stage, band edges, lights state) and active gains (kc, ki, threshold). Use to diagnose what the future continuous-intensity controller would have done; pair with `humidifier` stream to see divergence from the live bang-bang. See `docs/epics/continuous-humidifier/`. | 14 days | `apps/hwd/src/dirt_hwd/services/humidifier.py:HumidifierLoopService.run` (calls `apps/hwd/src/dirt_hwd/services/humidifier_pi.py:compute`) |
