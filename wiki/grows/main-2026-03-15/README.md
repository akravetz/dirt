---
title: Main Grow 2026-03-15
type: grow
sources: []
related: [wiki/overview.md, wiki/breeding/README.md, wiki/daily/2026-06-20.md, wiki/environment/humidity.md, wiki/environment/temperature.md, wiki/environment/nutrients.md]
created: 2026-05-30
updated: 2026-06-20
---

# Main Grow 2026-03-15

## Database Identity And Wiki Folder

| Field | Value |
|---|---|
| Wiki folder | `wiki/grows/main-2026-03-15/` |
| Folder role | Historical/documentation organization; not database identity |
| Site | `site.id = 1`, `site.name = Homebox` |
| Tent | `tent.id = 1`, `tent.name = Main Tent`, `tent.role = flower` |
| Current occupancy source | `plant_location_history.end_at IS NULL` |
| Current lifecycle source | `plant.germinated_at`, `plant.flower_started_at` |
| Line | Sirius Black x BS01 / SBxBS01 regular |
| Current plant count | 4 |

## Plants

- [Plant A](plants/plant-a.md) — database `plant.id = 1`, `plant.key = SBBS-R1-001`
- [Plant B](plants/plant-b.md) — database `plant.id = 2`, `plant.key = SBBS-R1-002`
- [Plant C](plants/plant-c.md) — database `plant.id = 3`, `plant.key = SBBS-R1-003`
- [Plant D](plants/plant-d.md) — database `plant.id = 4`, `plant.key = SBBS-R1-004`

## Current Role

This is the original four-plant SBxBS01 flower run. Plants A and D are the
primary purple keeper candidates; B and C remain secondary references. See the
current operational summary in [overview.md](../../overview.md).

As of 2026-06-20, the run is Day 98 / Flower Day 48. No daily photos were
available, so the latest visual canopy record remains 2026-06-19. Main VPD is
split again at 1.10 overnight / 1.39 morning / 1.55 now, with RH still above the
late-flower guide, so manual flower-pocket airflow checks remain active.

Today's trusted direct-percent moisture set reports only A/C/D: `SBBS-R1-001`
near 26.5%, `SBBS-R1-003` near 34.0%, and `SBBS-R1-004` near 31.4%, with only
minor movement from morning to now. `SBBS-R1-002` is omitted and remains
hand-check-only. Treat all moisture percentages as rough movement context and
use tray behavior, media condition, and posture for root-zone decisions.
