...
Plants A and D are strong candidates because they already show genetic purple expression and sativa-leaning structure. That does not mean their seeds will automatically reproduce the same phenotype.

An individual plant can look ideal while still being heterozygous at important loci. Purple expression may also depend on several genes plus environment. Backcrossing toward A/D increases average resemblance to A/D, but it does not guarantee every seed carries the complete purple/structure package.

The only way to know whether the target is becoming fixed is progeny testing: grow offspring, count how many match the target, and continue only the family with the highest on-target percentage.
...
If the backcross route is chosen, a practical feminized path is:

```
F1 regular male x A/D clone -> F2
best F2 female x reversed A/D pollen -> BX1 feminized
best BX1 female x reversed A/D pollen -> BX2 feminized
evaluate consistency, then decide whether to BX again, self, or return to F-line selection
```

Reversing A/D with STS is a valid way to make A/D pollen and produce feminized BX seed. The caution is that feminized BX seed still needs progeny testing. Repeatedly pollinating with A/D pollen pulls the line toward A/D, but it does not prove that dark-purple expression is homozygous or otherwise fixed.

## Accelerated generation cycle

Because yield is not the goal, generations can be run small and fast:
...
7. Pollinate selected candidates around flower day 14-21.
8. Harvest mature seed 4-6 weeks after pollination.

This can turn a generation in roughly 10-12 weeks from germination to mature seed, at the cost of yield and some final cured-flower confidence.

Avoid 12/12 from seed for the main line unless space forces it. A 2-3 week veg gives enough plant to judge structure, cut backups, and carry a useful seed branch.

## Space model

The efficient operating footprint is:

| Space | Role | Duration |
|---|---|---|
| Active grow space | Candidate population and selected females | Continuous |
| Male isolation space | Confirmed males only; evaluate, flower, collect pollen | Short-lived |
| Female clone shelf | Backup copies of promising females in stasis | Dynamic |

Males do not need to be maintained long-term. Keep only the best 2-4 candidate males, isolate them, collect pollen separately, then cull the plants. The frozen pollen bank becomes the male preservation system.

Female clones are still important. A female is often not proven until mid/late flower or even after progeny testing, when the original seed plant is no longer recoverable. The clone shelf prevents losing a plant that later turns out to matter.

Keep female clones in stasis, not as large mothers:

- solo cups or small nursery pots
- weak 18/6 light
- minimal feed
- aggressive pruning
- cull backups as soon as their matching flower plant fails

Practical target per generation:

- at flip: hold 6-12 candidate female clones if space allows
- flower week 4-6: reduce to 2-4 finalists
- after harvest/progeny decision: keep 1-2 elite clones only
...
6. Pollinate selected females with separate pollen lots so the next generation can be progeny-tested by family.

Do not bulk-mix pollen unless intentionally making a broad population. Separate family lots are what make progeny testing useful.
