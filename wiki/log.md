...
- **Root cause (research-grounded, see Raydrop [KC-RD05 manual](https://manuals.plus/raydrop/kc-rd05-humidifier-manual) + [RD05UPSTS support page](https://www.raydrop.net/pages/rd05upsts)):** low-water float sensor (manual part #12) latched empty even with water in the tank; controller red-lights and disables ultrasonic until a power cycle drops the latch. Usual trigger is mineral scale on the float stem — same substrate fouling the transducer, recommending distilled water + weekly vinegar descale. Raydrop has no thermal cutout and no other documented red-LED state, so this is the dominant failure mode.
