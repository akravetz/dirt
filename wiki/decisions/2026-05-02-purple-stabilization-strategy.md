...
Seeds from the eventual line should reliably produce plants with:
...
Use the first F2 as the variation-revealing generation, then prioritize progeny-tested F-generation selection over automatic backcrossing.

Preferred path:

```
selected SBxBS01 F1 male x A/D clone -> F2
select best purple/sativa F2 males and females -> F3 families
grow F3 progeny tests -> keep the family with the highest on-target percentage
repeat selection toward F4/F5
```
...
Backcrossing increases average resemblance to A/D, but it does not guarantee all offspring will express the target. Progeny testing is the actual stability test: grow offspring, measure the percentage that match the target, and continue only the family that passes the phenotype forward most consistently.
...
If A/D becomes the recurrent parent, a practical feminized backcross route is:
...
evaluate consistency; decide whether to BX again, self, or return to F-line selection
```

This is valid, but the resulting BX seed still needs progeny testing. It is a way to pull the line toward A/D, not proof of stabilized purple genetics.

## Consequences

- The second-pack decision should not drive the main line unless it supports the purple/sativa target.
- Population size matters more than yield per plant.
