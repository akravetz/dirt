...
- `apps/wake-word/data-gen/capture-rir-record.py` (Jabra host)
- `apps/wake-word/data-gen/capture-rir-play.py` (laptop)
...
7. Swap `var/wake-word/models/current/hey_claudia.onnx` → v4, restart service, run `apps/wake-word/validation/live-test.py` at close / far / meeting-ambient-background conditions.
...
- [ ] v4 `.onnx` deployed; before/after metrics on `apps/wake-word/validation/live-test.py`.
