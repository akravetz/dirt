...
**Date:** 2026-04-23
**Status:** **SUPERSEDED 2026-04-26** by [pivot to Govee H71xx Wi-Fi humidifier (deployed: H7142)](2026-04-26-govee-humidifier-pivot.md). The Raydrop MCU-mod path is abandoned; this decision is preserved for decision-trail history. The control-theory analysis below (PI vs PID, FOPDT, IMC tuning, RH-ceiling envelope guard) **carries over** to the Govee H7142 deployment — the H7142 just becomes the discrete-level actuator (`modeValue ∈ 1..9`) at the PI controller's dispatch boundary, with the same VPD-targeting design preserved. See the pivot decision for the integration shape. Original status before supersession was: Phase 1 (investigation) paused awaiting replacement spare; Phase 4 prep work landed end-to-end in shadow mode.
