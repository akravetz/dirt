...
| `heater` | Scheduled heater actuator telemetry. Kasa heater plugs emit `state_change` for on/off transitions with scope fields, `schedule_id`, `new_state`, `reason` (`scheduled_on` / `scheduled_off`), and `minutes_until_off` / `minutes_until_on`, plus `error` on loop exceptions. ThermoForge emits `status_read` (observed running/level plus target), `command_sent` (power or heat-level command), `state_change` (previous/new running and level), `poll_failed` (compact BLE exception with consecutive failure count and alert threshold), `offline` (debounced compact BLE exception plus next poll interval), and `recovered` (observed status after reachability returns). Kasa and ThermoForge schedules both come from scoped enabled `schedule` rows with `kind='heater'`, interpreted in each schedule timezone. | 30 days | `apps/hwd/src/dirt_hwd/services/kasa_schedule.py:ScheduledKasaActuatorService.run`; `apps/hwd/src/dirt_hwd/services/thermoforge.py:ScheduledThermoForgeService.run` |
