...
| `lights` | State transitions of Kasa plugs driving grow lights. One `state_change` event per on/off change with `reason` (`scheduled_on` / `scheduled_off`) and `minutes_until_off` / `minutes_until_on`. Also emits `error` events on loop exceptions. Schedules come from scoped enabled `schedule` rows with `kind='lights'`, interpreted in each schedule timezone. | 30 days | `apps/hwd/src/dirt_hwd/services/kasa_schedule.py:ScheduledKasaActuatorService.run` |
| `climate_controller` | Unified main-tent climate decisions. One `tick` event per control cycle with current sensor values, current/target actuator states, phase policy bands, VPD diagnostics, explicit demand fields (`raw_fan_rh_demand_pct`, `dehumidifier_capacity_request`, fan-burden/stable-off elapsed seconds, `lights_off_feedforward_bias_pct`, `lights_off_feedforward_rh_slope_pct_per_10m`), delivered output fields (`delivered_rh_drying_capacity_pct`, humidifier, heat), anti-windup tracking reasons, allocation/limit reasons, and command reason codes. Also emits `actuator_error` for command failures and `error` for tick failures. | 30 days | `apps/hwd/src/dirt_hwd/services/climate_controller.py:ClimateControllerService.run` |
