...
| `heat_pad` | State transitions of Kasa plugs driving heat pads. One `state_change` event per on/off change with breeding/main scope fields, `schedule_id`, `new_state`, `reason` (`scheduled_on` / `scheduled_off`), and `minutes_until_off` / `minutes_until_on`. Also emits `error` events on loop exceptions. Schedules come from scoped enabled `schedule` rows with `kind='heat_pad'`, interpreted in each schedule timezone. | 30 days | `apps/hwd/src/dirt_hwd/services/kasa_schedule.py:ScheduledKasaActuatorService.run` |
