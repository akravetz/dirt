...
| `lights` | State transitions of the Kasa plug driving the grow lights. One `state_change` event per on/off change with `reason` (`scheduled_on` / `scheduled_off`) and `minutes_until_off` / `minutes_until_on`. Also emits `error` events on loop exceptions. Schedule comes from the current scoped `growrun.lights_on_local` / `growrun.lights_off_local`, interpreted in `growrun.timezone`. | 30 days | `apps/hwd/src/dirt_hwd/services/lights.py:LightsLoopService.run` |
