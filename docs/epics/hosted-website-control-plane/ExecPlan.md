...
- [ ] Production Railway app deployment via `scripts/deploy-control-plane`. The first real API deployment failed during Railway build because Nixpacks selected Python 3.12.7; the next build succeeded but runtime could not import `dirt_control` because Railway's Nixpacks install phase ran before `src/` was copied. `apps/control-plane/.python-version`, `apps/control-plane/railway.json`, and the deploy smoke loop now cover those provider-specific gaps before redeploy.
...
Milestone 6 is locally complete and production database migration has begun through the supported deploy flow. The repository now includes `systemd/dirt-gateway.service`, `scripts/install-systemd` wiring, `scripts/deploy-control-plane`, `apps/control-plane/railway.json`, `docs/hosted-control-plane.md`, and command docs for supported Railway deployment and rollback. The cloud API exposes `/api/health` with gateway heartbeat age, local gateway backlog, command backlog/failure counts, asset failure counts, retention window, and command enablement. Cloud audit rows cover auth login success/failure, command creation, command claim, command result, asset upload completion/failure, asset retention, and gateway credential rotation. Cloud assets have a 30-day retention path: the gateway enqueues one daily `asset_retention` event, and the cloud API prunes expired `cloud_asset` rows plus S3 objects when bucket credentials are configured. The first production deploy run applied the cloud Atlas migrations, but replacement API containers did not reach production because Railway first built with Python 3.12.7 and then ran without `dirt_control` on `PYTHONPATH`. The API service now has an app-local `.python-version` pin for Python 3.13, a Railway start command that exposes `/app/src`, and a deploy script that waits for public smoke checks before moving to the UI deployment.
