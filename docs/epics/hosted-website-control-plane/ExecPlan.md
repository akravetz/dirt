...
- [ ] Production Railway app deployment via `scripts/deploy-control-plane`. The first real API deployment failed during Railway build because Nixpacks selected Python 3.12.7; `apps/control-plane/.python-version` now pins the service to Python 3.13 before redeploy.
