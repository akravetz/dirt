...
- [x] (2026-05-05T06:55:00Z) Final connection-pressure fix deployed: API deployment `e73f460b-6044-47a3-9709-0056ad850da1` and web-ui deployment `ea2c7480-bdf2-48c8-abdd-80bf8332b19b` are both `SUCCESS`; Railway liveness now probes `/healthz`, gateway credential `last_used_at` is updated by heartbeat only, repeated `/api/health` checks returned 200, and authenticated asset reads returned eight private S3 presigned assets.
- [x] (2026-05-05T07:03:00Z) Final local gateway scheduling fix deployed by restarting `dirt-gateway.service`: heartbeat/latest/assets now deliver ahead of rollup catch-up, `/api/health` stayed `status=live` across another sync interval, and local outbox state was down to delivered assets plus pending rollup catch-up rows.
...
- Observation: Hosted browser auth must not fall back to the local `AUTH_PASSWORD`.
  Evidence: Length-only checks originally showed `DIRT_CLOUD_ADMIN_PASSWORD_HASH` length `0` in `.env` and `.env.prod`, while the local `AUTH_PASSWORD` was present, and the deploy script briefly derived the cloud hash from that local password. Browser verification later showed this made the credential source of truth confusing once an explicit cloud hash existed. `scripts/deploy-control-plane` now requires `DIRT_CLOUD_ADMIN_USERNAME` and `DIRT_CLOUD_ADMIN_PASSWORD_HASH` explicitly and does not derive hosted credentials from local `AUTH_USERNAME` / `AUTH_PASSWORD`.
...
- Observation: The production API briefly exhausted its SQLAlchemy async connection pool while the gateway was catching up and command-claim polling continued.
  Evidence: Railway logs showed `QueuePool limit of size 5 overflow 10 reached` from gateway auth and `/api/gateway/v1/commands/claim`. The fix reduced gateway auth write pressure by moving credential `last_used_at` updates to heartbeat only and changed Railway's liveness probe from database-backed `/api/health` to process-only `/healthz`. After redeploy, repeated `/api/health` probes returned 200 and Railway logs showed `/healthz`, command claims, and `/api/health` returning 200 without new pool errors.

- Observation: Gateway health can become stale if old rollup retry rows are delivered before newly enqueued heartbeat rows.
  Evidence: Local `cloud_gateway` logs showed the gateway loop spending multiple 20-second request timeouts on historical `rollups` rows before delivering heartbeat, which made `/api/health` report `stale` even though `dirt-gateway.service` was running. The gateway outbox now delivers heartbeat/latest/catalog/assets/command results first and retries at most one rollup batch per cycle; after restarting the service, `/api/health` stayed `live` across another interval.
...
- Gateway prioritization validation passed: `uv run pytest apps/gateway/tests/test_sync.py -q` passed with `12 passed`; `uv run pytest apps/gateway/tests apps/control-plane/tests -q` passed with `32 passed`; `uv run ruff check apps/gateway apps/control-plane` passed; `uv run ruff format apps/gateway apps/control-plane --check` passed.
- Final local gateway restart picked up the prioritization fix. `systemctl --user restart dirt-gateway` succeeded, three repeated `/api/health` probes returned 200, and a later check after another sync interval still returned `status=live` with `gateway_heartbeat_age_s=48`.
- Final residual gateway backlog is rollup catch-up only: local outbox showed delivered `asset_upload=10`, delivered `heartbeat=19`, delivered `latest_metrics=19`, delivered `rollups=2`, and pending `rollups=17`; cloud health reported `gateway_backlog_depth=17`, zero command failures, and `asset_failures_24h=18` from earlier failed upload attempts.
...
- 2026-05-05 / Codex: Reduced cloud API database pressure after production health probes exposed transient pool exhaustion, redeployed through the supported script, and verified stable live health plus private signed asset access.
- 2026-05-05 / Codex: Prioritized gateway outbox delivery so heartbeat/latest state is not blocked behind historical rollup retries, restarted the local gateway, and verified cloud health stayed live.
