...
- [x] (2026-07-04 07:25Z) Implemented Milestone 4: tests, docs, and operator validation checklist. Added focused backend/API boundary tests, command/wiki operator docs, and build/lint/full-suite validation. Hardware curl validation remains pending until the controller is flashed and state-changing calibration endpoints are intentionally exercised.
...
- Observation: The software/docs milestone can be completed without mutating the physical controller, but operator validation still needs a flashed device.
  Evidence: Milestone 4 validation passed for focused tests, full pytest, firmware builds, ruff, wiki lint, and diff whitespace checks. The `curl` checks for `/calibration/start`, `/samples`, `/calibration/stop`, and observed 30-second Dirt ingest cadence were not run because they require flashing the controller and intentionally changing its calibration-mode state.
...
Milestone 4 added focused coverage for multi-capture dry/wet formula averaging, partial completion warnings, completed-session immutability, temp-root storage through `DIRT_DATA_DIR`, malformed firmware `/samples` validation, and the local FastAPI live-status/preview/accept/complete/summary response-model flow. Documentation now includes the local calibration command in `docs/commands.md`, controller calibration endpoint and probe-mapping notes in `wiki/hardware/rs485-substrate-sensors.md`, and the dry 70/30 coco/perlite plus wet field-capacity operator workflow in `wiki/hardware/rs485-substrate-sensor-calibration.md`. Validation passed for `uv run pytest apps/hwd/tests/test_substrate_calibration.py -q`, `uv run ruff check apps/hwd/src/dirt_hwd/tools/substrate_calibration apps/hwd/tests/test_substrate_calibration.py`, `uv run ruff format --check apps/hwd/src/dirt_hwd/tools/substrate_calibration apps/hwd/tests/test_substrate_calibration.py`, `uv run pytest -q`, `pio run -e plant-a-substrate`, `pio run -e plant-a-substrate-ota`, `uv run scripts/lint.py`, and scoped `git diff --check`. Wiki lint reported file-length warnings only.

Remaining operator validation: flash the RS485 controller through the approved firmware workflow, then run the documented `curl` checks for `/health`, `/status`, `/calibration/start`, `/samples`, and `/calibration/stop`, and confirm Dirt ingest remains near the normal 30-second cadence while calibration mode is active.
...
- 2026-07-04: Completed the implementation milestones through software validation and operator documentation. Hardware endpoint validation remains as an explicit post-flash operator step.
