...
- [x] (2026-06-21 19:53 MDT) Milestone 5 complete: added seed-lot detail and inventory update browser APIs, explicit seed-lot detail/update DTOs, the typed `breeding_seed_lot_update` cloud command payload, gateway local execution as `breeding.seed_lot.update`, focused shared/gateway/control-plane tests, and regenerated the hosted browser contract.
- [x] (2026-06-21 20:25 MDT) Milestone 6 complete: split Seeds into routable list/create/detail/edit pages, moved seed-owned UI/query/mutation/type code into `web-ui/src/features/seeds`, added URL-backed seed inventory filters, wired seed-lot detail/update to the generated hosted API contract, preserved command-backed pending/convergence UX, and removed the temporary Plants-owned seed creation bridge.
- [x] (2026-06-21 20:38 MDT) Milestone 7 complete: cleaned stale frontend-owned Breeding Logbook cache/helper names, validated the final route tree with full focused checks, restarted the local dev stack so browser verification used current backend code, and captured final desktop/mobile light/dark screenshots for the Tents, Plants, and Seeds workspaces.
...
- Observation: Local browser verification can hit stale backend code if the dev stack predates new FastAPI routes.
  Evidence: the first Milestone 7 browser check of `/seeds/2` rendered the new frontend route but `GET http://192.168.1.79:8021/api/breeding-logbook/seed-lots/2` returned `{"detail":"Not Found"}` from the long-running API process. After `make dev-down && make dev-up`, the same endpoint returned 200 and the seed-lot detail page rendered.
...
Milestone 5 added backend and contract support for seed-lot detail/edit. The new detail route returns summary fields plus line identity, purchased acquisition facts, cross context, notes, seed count, and created-plant count, with 404 for missing seed lots. The new update route validates the projected seed lot and enqueues a typed complete-state inventory command. The gateway applies that command to local `SeedLot` rows only, validates sex type, requires `vendor_name` for purchased lots, rejects purchased-only fields for cross-created lots, and reports `source_seed_lot_id` in the result. Validation passed with `DIRT_CLOUD_ASSET_STORE=local scripts/gen-hosted-contract`, `uv run pytest apps/shared/tests/test_cloud_contract.py -q`, `uv run pytest apps/gateway/tests/test_sync.py apps/gateway/tests/test_gateway_boundary_guardrails.py -q`, `uv run pytest apps/control-plane/tests/test_api.py apps/control-plane/tests/test_control_plane_boundary_guardrails.py -q`, `pnpm --dir web-ui typecheck`, and `git diff --check`.

Milestone 6 replaced Seeds placeholders with real inventory workflows. `/seeds` now lists and filters seed lots from hosted read models; `/seeds/new` enqueues seed-lot creation without assuming an immediate projected id; `/seeds/$seedLotId` and `/seeds/$seedLotId/edit` use the generated detail/update contract and keep line/cross identity read-only. Plants still supports germination from seed lots and links to `/seeds/new`, but no longer owns seed-lot creation UI or mutations. Validation passed with `pnpm --dir web-ui exec vite build`, `pnpm --dir web-ui typecheck`, `pnpm --dir web-ui lint`, `pnpm --dir web-ui test`, `pnpm --dir web-ui knip --no-progress`, `uv run pytest apps/tests/invariants/test_typescript_dead_code.py -q`, focused stale-reference searches, and `git diff --check`.

Milestone 7 removed remaining stale frontend-owned Breeding Logbook names by moving query/idempotency namespaces to `plants` and `seeds`, renaming the shared route leaf helper from `-workspacePlaceholders` to `-leafRoute`, and leaving generated/backend `/api/breeding-logbook/...` contract paths intact. Validation passed with `pnpm --dir web-ui exec vite build`, `pnpm --dir web-ui typecheck`, `pnpm --dir web-ui lint`, `pnpm --dir web-ui test`, `pnpm --dir web-ui knip --no-progress`, `uv run pytest apps/tests/invariants -q`, `uv run pytest apps/shared/tests/test_cloud_contract.py -q`, `uv run pytest apps/gateway/tests/test_sync.py apps/gateway/tests/test_gateway_boundary_guardrails.py -q`, `uv run pytest apps/control-plane/tests/test_api.py apps/control-plane/tests/test_control_plane_boundary_guardrails.py -q`, focused stale-route/string searches, and `git diff --check`. Browser verification used the real local auth session at `http://192.168.1.79:5171`, confirmed `/` redirects to `/tents/1`, and exercised `/tents`, `/tents/1`, `/plants`, `/plants/new`, `/plants/SBBS-R1-001`, `/plants/SBBS-R1-001/edit`, `/seeds`, `/seeds/new`, `/seeds/2`, and `/seeds/2/edit` with no browser errors. Final screenshots are in `debug/workspace-reorientation-screens/` for desktop/mobile and light/dark coverage.
