...
Milestone 5 implementation is complete in source, schema migration, tests, and docs, but it has not been applied to the live database. Current app code now assumes the Milestone 5 migration has been applied: HWD ingest requires `device_id`, readings are inserted with `capability_id` only, and current tests no longer seed `sensornode_id`. A follow-up invariant cleanup removed the final derived `EMITTED_METRICS` / `PERSISTED_METRICS` compatibility exports from `sensor_contract.py`. Before running the live apply, take the normal `pg_dump` backup, then apply `20260504144109_scoped_firmware_legacy_removal.sql`.
