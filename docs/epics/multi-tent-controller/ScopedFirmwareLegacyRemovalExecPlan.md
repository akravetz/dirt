...
- [x] (2026-05-04) Milestone 4: replace remaining service inventories that are keyed by `SensorLocation`. Daily report sensor validation/snapshot now discovers default-main tent requirements from `fan-controller` persisted capability contracts plus live capability rows, and plant moisture requirements from current `plant.moisture_capability_id` rows. Metric freshness state/logging now keys by `device_id:capability_id` and emits scoped fields without `location`. Voice sensor tools read default-main tent metrics with `device_id='fan-controller'` and plant moisture through current scoped plant capabilities. Production service code no longer imports legacy `SensorLocation` maps; a later invariant cleanup removed the temporary derived legacy metric maps.
- [x] (2026-05-04) Milestone 5 final schema cleanup applied live. SQLModel source now removes `SensorLocation`, `SensorNode`, `sensornode`, and `sensorreading.sensornode_id`; HWD ingest requires `device_id`; `ReadingsService` writes capability-owned readings only; current web/voice/daily-report tests seed/query capabilities directly. Atlas migration `20260504144109_scoped_firmware_legacy_removal.sql` converted historical `reservoir_depth_cm` to canonical `reservoir_in` with `value / 2.54`, deleted known trash null-capability tent `pressure_hpa` and plant-a `humidity_pct`, guarded against any remaining null-capability rows, then dropped the legacy column/table/type. Live apply followed `pg_dump` backup `var/db-backups/dirt-20260504-092029-pre-scoped-firmware-legacy-removal.sql`; services restarted cleanly with fresh scoped firmware ingest.
...
- Observation: Human-owned invariants still import the legacy sensor-contract maps.
  Evidence: `uv run pytest apps/tests/invariants/ -q` initially failed during collection because `apps/tests/invariants/test_sensor_contract.py` imported `EMITTED_METRICS` and `PERSISTED_METRICS`. Those maps were temporarily kept as compatibility exports derived from `DEVICE_METRICS`, then removed after explicit user permission to update the invariant to assert canonical `DEVICE_METRICS` directly.
...
- Observation: The sensor-contract invariant was the last consumer of the derived legacy metric maps.
  Evidence: Milestone 5 initially kept invariant-only `EMITTED_METRICS` / `PERSISTED_METRICS` maps keyed by a private `ContractLocation`. A follow-up cleanup removed `ContractLocation` and both maps after `apps/tests/invariants/test_sensor_contract.py` was updated to validate `DEVICE_METRICS`, `emitted_metrics_for_device_id()`, `persisted_metrics_for_device_id()`, and `persisted_capability_ids_for_device_id()` directly.
...
Milestone 4 is complete. Daily report, metric freshness, and voice sensor summaries no longer use service inventories keyed by `SensorLocation`. The remaining `SensorLocation` references at that point were compatibility/model surfaces for the still-live `sensornode` and `sensorreading.sensornode_id` schema, the HWD ingest compatibility bridge, the humidifier's legacy internal recording path, centralized readings compatibility helpers, and temporary invariant compatibility. `sensor_contract.py` stopped exporting `LEGACY_LOCATION_DEVICE_IDS`, `emitted_metrics()`, and `persisted_metrics()`.

Milestone 5 is complete in source, schema migration, tests, docs, and the live database. The live apply was preceded by `pg_dump` backup `var/db-backups/dirt-20260504-092029-pre-scoped-firmware-legacy-removal.sql`, migrated Atlas to `20260504144109`, removed legacy `sensornode` storage, and left current ESP32 ingest flowing through scoped `device_id` / `capability_id` ownership. A follow-up invariant cleanup removed the final derived `EMITTED_METRICS` / `PERSISTED_METRICS` compatibility exports from `sensor_contract.py`.


## Context and Orientation

The previous plan, `docs/epics/multi-tent-controller/LegacyCompatibilityRetirementExecPlan.md`, completed the first compatibility-retirement phase. The live database is migrated through `20260504144109`. `Plant.sensornode_id`, `SensorCalibration.sensornode_id`, `GrowRun.location`, duplicated grow-run photoperiod columns, `SensorReading.sensornode_id`, `sensornode`, and `sensor_location` are gone. Current plant moisture, calibration, grow-current, ingest, and lights behavior use capability and schedule ownership.
...
- `apps/hwd/src/dirt_hwd/api/ingest.py` requires scoped `device_id`; legacy `location` is no longer part of the current request model.
- `apps/shared/src/dirt_shared/services/readings.py` writes capability-owned `SensorReading` rows only.
- `apps/shared/src/dirt_shared/sensor_contract.py` declares canonical `DEVICE_METRICS` plus scoped helper functions; derived legacy metric maps have been removed.
- `apps/shared/src/dirt_shared/services/daily_sensors.py`, `apps/hwd/src/dirt_hwd/services/metric_freshness.py`, and `apps/voice/src/dirt_voice/tools/sensors.py` use scoped device/capability inventories.
...
- Legacy-only ingest means an old ESP32 payload that supplies `location` but no `device_id`; current HWD ingest rejects it.
...
- `sensornode`, `sensor_location`, and `sensorreading.sensornode_id` are absent after the applied Milestone 5 Atlas migration.
...
- 2026-05-04: Milestone 5 source/schema/test/doc implementation completed without live Atlas apply. The cleanup migration performs the approved historical conversion/deletion policy and drops the legacy table/column/type.
- 2026-05-04: Live Milestone 5 Atlas apply completed after `pg_dump` backup `var/db-backups/dirt-20260504-092029-pre-scoped-firmware-legacy-removal.sql`. Validation showed Atlas at `20260504144109`, zero null-capability readings, no `sensornode` table, no `sensor_location` type, no `sensorreading.sensornode_id`, and fresh post-restart scoped firmware ingest from all six ESP32 devices.
