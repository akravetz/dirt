...
| [tanstack-query-v5/](tanstack-query-v5/INDEX.md) | writing or modifying TanStack Query hooks/imports in `web-ui/src/`, defining query keys/options, calling `useQuery` / `useSuspenseQueries` / `useMutation` / `useQueries`, using `QueryClient`, invalidation/refetch, optimistic/cache writes, or command-backed browser state |
...
- **TanStack Query v5**: training data reaches for `useEffect` data fetching, loose string keys, normalized-cache assumptions, in-place cache mutation, broad invalidation, or invalidation-only command UI. Use array keys, query option factories, targeted invalidation, immutable cache writes, pending variables, and `onMutate` rollback patterns.
