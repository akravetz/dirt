...
4. **Lifecycle**: `POST /pods` with `dockerStartCmd: ["python", "train.py"]` → poll `GET /pods/{id}` until `desiredStatus == "EXITED"` → SCP artifacts off (Pod must have a public IP — request `supportPublicIp: true` and `cloudType: "SECURE"`) → `DELETE /pods/{id}`. See [pod-lifecycle.md](pod-lifecycle.md).
...
- **[pod-lifecycle.md](pod-lifecycle.md)** — `POST /pods` → `GET /pods/{id}` polling → `DELETE /pods/{id}`. Status enum, what `desiredStatus` actually means (no exit code is exposed), and the "phantom RUNNING" gotcha when the container has crashed but the Pod is still leased.
...
- ❌ **Assuming a returned exit code from `GET /pods/{id}`**. The REST `desiredStatus` enum is just `RUNNING` / `EXITED` / `TERMINATED`. There is no `exitCode` field. To know whether your training script succeeded, write a sentinel file (e.g. `/workspace/out/SUCCESS`) at the end of your entrypoint and check for it after `EXITED`. See [pod-lifecycle.md](pod-lifecycle.md).
