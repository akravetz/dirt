...
| `tanstack-query-v5/` | `web-ui/src/` TanStack Query hooks/imports, query keys/options, `useQuery`/`useSuspenseQueries`/`useMutation`/`useQueries`, optimistic/cache writes, invalidation/refetch, command-backed browser state |
...
- **Hosted frontend contract**: hosted browser API types are generated from `apps/control-plane` FastAPI OpenAPI with `scripts/gen-hosted-contract`. Do not add handwritten hosted response interfaces in `web-ui/src/api-client/cloud.ts`; regenerate and consume `web-ui/src/api-client/generated/hosted-schema.ts`.
...
- **Simple clean architecture**: Clean architecture means the simplest truthful model, not more architecture. Prefer direct, explicit code and data; add abstractions only for real shared responsibility; choose direct cutover for source-owned code; do not keep dead wrappers or compatibility layers. Read and follow [`docs/rules/simple-clean-architecture.md`](docs/rules/simple-clean-architecture.md) before planning architecture, adding abstractions, or preserving compatibility.
