...
The agent reads this file once when pulled in by the CLAUDE.md pointer. From it, the agent must be able to decide (a) whether the pack is relevant to the task at hand, and (b) which topic file(s) to pull next.
...
claude_md_pointer:
  ...
  consult_when: <the specific consult-when phrase you wrote into CLAUDE.md>
```

The `claude_md_pointer.consult_when` field is a record of what you wrote into CLAUDE.md — useful if a future refresh needs to regenerate that block.
